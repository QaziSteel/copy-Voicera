...
DROP POLICY IF EXISTS "Allow booking insertion for external systems" ON public.bookings;
...
CREATE POLICY "View bookings through project relationship" 
ON public.bookings 
FOR SELECT 
USING (
...
-- Allow inserting bookings without project_id requirement (for external systems)
CREATE POLICY "Insert bookings for external and internal systems" 
ON public.bookings 
FOR INSERT 
WITH CHECK (
  -- Either no auth (external system) or authenticated user with project access
  auth.uid() IS NULL OR 
...
CREATE POLICY "Update bookings by project owners" 
ON public.bookings 
FOR UPDATE 
USING (
  auth.uid() IS NOT NULL 
...
CREATE POLICY "Delete bookings by project owners" 
ON public.bookings 
FOR DELETE 
USING (
  auth.uid() IS NOT NULL 
...
-- Create trigger function to automatically populate project_id after insert
...
BEGIN
  -- Only populate if project_id is not already set and we have a call_log_id
  IF NEW.project_id IS NULL AND NEW.call_log_id IS NOT NULL THEN
    -- Get project_id from call log relationship
    SELECT pn.project_id INTO derived_project_id
    FROM call_logs cl
    LEFT JOIN phone_numbers pn ON (cl.phone_number_id = pn.id OR cl.phone_number = pn.phone_number)
    WHERE cl.id = NEW.call_log_id
    LIMIT 1;
...
-- Create trigger that runs before insert to populate project_id
DROP TRIGGER IF EXISTS populate_booking_project_id_trigger ON public.bookings;
CREATE TRIGGER populate_booking_project_id_trigger
