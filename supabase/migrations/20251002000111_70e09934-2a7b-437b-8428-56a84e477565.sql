-- Drop all versions of encryption-related functions to ensure clean state
DROP FUNCTION IF EXISTS public.encrypt_oauth_token(text, uuid);
DROP FUNCTION IF EXISTS public.encrypt_oauth_token(text, uuid, text);
DROP FUNCTION IF EXISTS public.decrypt_oauth_token(text, uuid);
DROP FUNCTION IF EXISTS public.decrypt_oauth_token(text, uuid, text);
DROP FUNCTION IF EXISTS public.get_google_integration_tokens(uuid, uuid);
DROP FUNCTION IF EXISTS public.get_google_integration_tokens(uuid, uuid, text);
DROP FUNCTION IF EXISTS public.store_google_tokens(uuid, text, text, timestamp with time zone, uuid);
DROP FUNCTION IF EXISTS public.store_google_tokens(uuid, text, text, timestamp with time zone, uuid, text);
DROP FUNCTION IF EXISTS public.update_encrypted_access_token(uuid, text, timestamp with time zone, uuid);
DROP FUNCTION IF EXISTS public.update_encrypted_access_token(uuid, text, timestamp with time zone, uuid, text);

-- Recreate encrypt_oauth_token with encryption key parameter
...
-- Recreate decrypt_oauth_token with encryption key parameter
...
-- Recreate get_google_integration_tokens with encryption key parameter
...
BEGIN
  ...
  -- Return decrypted tokens using the encrypted columns
  RETURN QUERY SELECT 
...
-- Recreate store_google_tokens with encryption key parameter
...
-- Recreate update_encrypted_access_token with encryption key parameter
